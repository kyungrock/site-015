#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""블로그 글 추가 후 HTML·sitemap 재생성 (site-015)"""
...
def write_sitemap(domain: str) -> None:
    ...
    if posts_dir.is_dir():
        for html_path in sorted(posts_dir.glob("*.html")):
            rel = f"posts/{html_path.name}"
            lines.append(
                f'  <url><loc>{domain}/{rel}</loc><priority>0.8</priority></url>'
            )
    lines.append("</urlset>\n")
...
def main() -> None:
    ...
    print(f"블로그 페이지 {n}개 생성 (blog.html + 글 {max(0, n - 1)}개)")
    print("index.html 홈 전체글·사이드바 갱신 완료")
    print("sitemap.xml, robots.txt 갱신 완료")
    print("\nDeno Deploy 배포:")
    print('  git add . && git commit -m "blog publish" && git push')
